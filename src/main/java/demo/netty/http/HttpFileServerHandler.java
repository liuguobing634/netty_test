...
public class HttpFileServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        RandomAccessFile randomAcceddFile = null;
        try {
            randomAcceddFile = new RandomAccessFile(file,"r");

        }catch (FileNotFoundException fnfe) {
            sendError(ctx,HttpResponseStatus.NOT_FOUND);
            return;
        }
        long fileLength = randomAcceddFile.length();
        ...
        sendFileFuture = ctx.write(new ChunkedFile(randomAcceddFile,0,fileLength,8192),ctx.newProgressivePromise());
